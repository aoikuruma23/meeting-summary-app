...
def _build_verification_html(subject: str, code: str) -> str:
    return f"""
            <h2>{subject}</h2>
...
def _send_with_resend(to_email: str, subject: str, html: str, api_key: str) -> bool:
    try:
        import resend

        from_email = (os.getenv("RESEND_FROM_EMAIL") or "noreply@jibunkaikaku-lab.com").strip()
        print(f"DEBUG: Resend key prefix={api_key[:6]}... suffix=...{api_key[-4:]} len={len(api_key)} from={from_email}")
        ...
        if sent_id:
            print(f"DEBUG: Resend送信成功 - {to_email}, id={sent_id}")
            return True
        print(f"DEBUG: Resend応答異常 - {to_email}, response={response!r}")
        return False
    except Exception as e:
        body = getattr(e, "body", None) or getattr(e, "message", None)
        status = getattr(e, "status_code", None) or getattr(e, "code", None)
        print(f"DEBUG: Resend送信エラー - {to_email}: {e} | status={status} body={body!r}")
        return False


def _send_with_sendgrid(to_email: str, subject: str, html: str) -> bool:
    try:
        from sendgrid import SendGridAPIClient
        from sendgrid.helpers.mail import Mail

        # 環境変数の前後空白/改行を除去（コピペ事故対策）
        api_key = (os.getenv("SENDGRID_API_KEY") or "").strip()
        from_email = (os.getenv("SENDGRID_FROM_EMAIL") or "noreply@yourdomain.com").strip()

        if not api_key:
            print("DEBUG: SendGrid APIキーが設定されていません")
            return False

        # 値の妥当性を可視化（先頭6/末尾4のみ・長さ）。秘密本体は出さない
        print(f"DEBUG: SendGrid key prefix={api_key[:6]}... suffix=...{api_key[-4:]} len={len(api_key)} from={from_email}")
        ...
        if response.status_code in [200, 201, 202]:
            print(f"DEBUG: SendGrid送信成功 - {to_email}")
            return True
        print(f"DEBUG: SendGrid送信失敗 - {to_email}, ステータス: {response.status_code}")
        return False

    except Exception as e:
        # SendGrid (python_http_client) の HTTPError は .body / .status_code を持つ
        body = getattr(e, "body", None)
        status = getattr(e, "status_code", None)
        headers = getattr(e, "headers", None)
        print(f"DEBUG: SendGrid送信エラー - {to_email}: {e} | status={status} body={body!r} headers={headers}")
        return False


def send_verification_email(email: str, code: str, *, subject: str = "メールアドレス確認") -> bool:
    """RESEND_API_KEY があれば Resend、無ければ SendGrid にフォールバック。"""
    html = _build_verification_html(subject, code)

    resend_key = (os.getenv("RESEND_API_KEY") or "").strip()
    if resend_key:
        return _send_with_resend(email, subject, html, resend_key)

    return _send_with_sendgrid(email, subject, html)
